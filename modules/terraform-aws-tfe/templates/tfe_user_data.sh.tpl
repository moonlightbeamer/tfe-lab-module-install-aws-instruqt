...
configure_log_forwarding_cloudwatch() {
  echo "INFO: Configuring Fluent Bit log forwarding with AWS CloudWatch log group target."
...
configure_log_forwarding_s3() {
  echo "INFO: Configuring Fluent Bit log forwarding with AWS S3 target."
  cat > "$TFE_CONFIG_DIR/fluent-bit.conf" << EOF
[OUTPUT]
    Name                         s3
    Match                        *
...
main() {
  ...
  if [[ "${log_forwarding_enabled}" == "1" && "${log_type}" == "cloudwatch_logs" ]]; then
    configure_log_forwarding_cloudwatch
  elif [[ "${log_forwarding_enabled}" == "1" && "${log_type}" == "s3" ]]; then
    configure_log_forwarding_s3
  else
